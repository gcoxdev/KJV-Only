...
for (const width of [390, 1280]) {
  test(`chapter context, figures, evidence and expanded layout at ${width}px`, async ({ page }) => {
    ...
    await expect(dialog.getByRole("article")).toContainText("External sources and citation details are listed on the Credits page.");
...
for (const width of [390, 1280]) {
  test(`poetry and wisdom refinement evidence at ${width}px`, async ({ page }) => {
    ...
    await expect(evidence).toContainText("Psalm 3 heading");
    await expect(evidence).toContainText("A Psalm of David, when he fled from Absalom his son.");
    await expect(evidence.locator('a[href^="http"]')).toHaveCount(0);
    ...
    await expect(evidence).toContainText("Dates unknown");
    await expect(evidence).toContainText("Psalm 51 heading");
    await expect(evidence).toContainText("when Nathan the prophet came unto him");
    await expect(evidence.locator('a[href^="http"]')).toHaveCount(0);
