...
it("validates the reviewed ranges and all New Testament chapter references against the shipped KJV", () => {
  ...
  for (const book of books.slice(39)) {
  ...
  const citations = [...WRITING_CONTEXTS.flatMap(w => w.references), ...Object.values(CHAPTER_TIMELINE_MAP).flatMap(chapters => Object.values(chapters).flatMap(mapping => mapping.references ?? []))];
