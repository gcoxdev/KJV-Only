...
function serializeLeaf(
  ...
  if (node.view === "reader") {
    ...
    return `${chapterValue}.${serializeVerseRanges(ranges)}`;
  }
  if (node.view === "page" && node.pageId) {
...
function serializeNode(
  ...
  if (node.type === "leaf") {
    return serializeLeaf(node, highlightedVerseRangesByLeafId);
  }
  const orientation = node.orientation === "horizontal" ? "h" : "v";
  const ratio = Math.max(1, Math.min(99, Math.round(node.ratio)));
  return `${orientation}${ratio}(${serializeNode(node.first, highlightedVerseRangesByLeafId)};${serializeNode(node.second, highlightedVerseRangesByLeafId)})`;
}

function parseLeafToken(token: string): [LeafNode | null, SerializedVerseRange[]] {
  if (token === "search") {
    return [{ ...createReaderLeaf(0, 0, "search"), pageId: null }, []];
  }
  if (token === "notes") {
    ...
    return [
      {
        ...
        pageId: token.slice("page.".length) as LeafNode["pageId"],
      },
      [],
    ];
  }

  const match = token.match(/^([1-3A-Z]{3})\.(\d+)(?:\.(.+))?$/);
  if (!match) {
    return [null, []];
  }
...
function parseNode(
  ...
  const [leaf, ranges] = parseLeafToken(token);
...
export function serializeLayoutHash(args: {
  ...
  const tabSegments = args.tabs.map((tab) => {
    ...
    return `${encodedTitle}:${serializeNode(tab.root, highlightedVerseRangesByLeafId)}`;
  });
...
export function parseLayoutHash(hash: string): ParsedLayoutHash | null {
  ...
  const tabs = tabSegments
    .map((segment, index): ReaderTab | null => {
      ...
      const [root, endIndex] = parseNode(treeValue, highlightedVerseRangesByLeafId);
